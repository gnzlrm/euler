/**
Code by Gonzalo Rolon Morinelli, Dec 19 2015.
...
function parseGrid(gridString) {
    ...
    for (i = 0; i < grid.length; i++) {
        line = grid[i].split(' ');
        grid[i] = line.map(parseElement);
    }
    return grid;
}

function parseElement(e) {
    // Parse an element from string to integer. Wrapper for parseInt to use with map method.
...
function getMaxProductByDir(grid, n, stepH, stepV, startH) {
    ...
    of n consecutive numbers in a direction specified by paremeters stepH and
    stepV.
    */
    var maxProd = Number.NEGATIVE_INFINITY;
    var offsetV = (stepV > 0) ? n - 1 : 0;
    var offsetH = (stepH > 0) ? n - 1 : 0;
    for (i = 0; i < grid.length - offsetV; i++) {
        for (j = startH; j < grid.length - offsetH; j++) {
            nList = [];
            for (k = 0; k < n; k++) {
                nList.push(grid[i + (stepV * k)][j + (stepH * k)]);
            }
            var prod = getProduct(nList);
            if (prod > maxProd) {
                maxProd = prod;
            }
        }
...
function getMaxProdInGrid(sGrid, n) {
    /**
    ...
    diagonally.
    */
    grid = parseGrid(sGrid);
    return Math.max(getMaxProductByDir(grid, n, 1, 0, 0),
                    getMaxProductByDir(grid, n, 0, 1, 0),
                    getMaxProductByDir(grid, n, 1, 1, 0),
                    getMaxProductByDir(grid, n, -1, 1, n - 1));
}

module.exports.parseGrid = parseGrid;
module.exports.parseElement = parseElement;
module.exports.getProduct = getProduct;
module.exports.getMaxProductByDir = getMaxProductByDir;
module.exports.getMaxProdInGrid = getMaxProdInGrid;

if (require.main === module) {
    // Problem solution.
    console.log(getMaxProdInGrid(
