"""Code by Gonzalo Rolon Morinelli, Nov 25 2015.
...
def get_max_product_by_dir(grid, n, step_h, step_v, start_h=0):
    """Get the maximum product of n numbers in a grid.
    ...
    """
    max_prod = float('-inf')
    offset_v = n - 1 if step_v > 0 else 0
    offset_h = n - 1 if step_h > 0 else 0
    for idx in range(len(grid) - offset_v):
        for jdx in range(start_h, len(grid) - offset_h):
            num_list = [grid[idx + step_v * i][jdx + step_h * i]
                        for i in range(n)]
            prod = get_product(num_list)
            if prod > max_prod:
                max_prod = prod
    return max_prod
...
def get_max_product_in_grid(grid_string, n):
    ...
    return max(get_max_product_by_dir(grid, n, 1, 0),
               ...
               get_max_product_by_dir(grid, n, -1, 1, n - 1))

if __name__ == '__main__':
    # Problem solution
    print get_max_product_in_grid(
